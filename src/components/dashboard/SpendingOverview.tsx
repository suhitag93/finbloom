...
const SpendingOverview = () => {
  ...
  return (
    ...
    >
      ...
      <p className="font-display text-2xl font-semibold text-foreground mb-1">${displayTotal.toLocaleString()}</p>
      <p className="text-sm text-muted-foreground mb-4">
        {categories.length > 0 ? `${categories.length} categories tracked` : "Connect accounts to see real data"}
      </p>

      {/* Stacked bar – full width, max 200px height concept but bar is thin */}
      ...
      <div className="space-y-3">
        {displayCategories.map((cat) => (
          <div key={cat.name} className="flex items-center justify-between min-h-[44px]">
